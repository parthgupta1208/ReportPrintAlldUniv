...
def update():
    ...
    year_selected=str(menu1.get())[2:4]
    tab1=year_selected+course_selected
    filendat=e41.get()
    filnmdat=e42.get()
    filgndat=e43.get()
    filmndat=e44.get()
...
def reportopen():
    ...
    access.DoCmd.OpenReport(course_selected,0)
...
def but2():
    ...
    b34=Button(root3,text="Print",command=but3)
...
def but1(*args):
    ...
    tablenames1=[]
    filename=str(os.getcwd())+'\\'+str(menu1.get())+'_data.accdb'
    try:
    ...
    options2 = [x[2:] for x in tablenames1 if x[0:1].isnumeric()]
    drop2 = OptionMenu(root,menu2,*options2).grid(row=2,column=2,sticky="news",padx=10,pady=10)

root=Tk()
root.title("Choose Year")
l1=Label(text="Select Year").grid(row=1,column=1,sticky="news",padx=10,pady=10)
options1 = [x[2:6] for x in glob.glob('./*.accdb')]+[x[2:6] for x in glob.glob('./*.mdb')]
menu1= StringVar()
menu1.set(options1[0])
drop1 = OptionMenu(root,menu1,*options1 ).grid(row=1,column=2,sticky="news",padx=10,pady=10)
menu1.trace("w",but1)
l2=Label(root,text="Select Course").grid(row=2,column=1,sticky="news",padx=10,pady=10)
options2 = ["---"]
menu2= StringVar()
